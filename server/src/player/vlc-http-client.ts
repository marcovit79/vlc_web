...
export type State = 'paused' | 'playing' | 'stopped';
...
export type StatusPaused = StatusBase & {
  stats: Stats;
  aspectratio: AspectRatio;
  state: 'paused';
};

export type StatusPlaying = StatusBase & {
  stats: Stats;
  aspectratio: AspectRatio;
  state: 'playing';
};

export type StatusStopped = StatusBase & {
  stats: null;
  aspectratio: null;
  state: 'stopped';
};

export type Status = StatusPaused | StatusPlaying | StatusStopped;
...
export type PlaylistLeaf = PlaylistBase & {
  duration: number;
  uri: string;
};

export type PlaylistNode = PlaylistBase & {
  children: (PlaylistNode | PlaylistLeaf)[];
};

export type Playlist = PlaylistNode | PlaylistLeaf;

export type VLCOptions2 = {
  host?: string;
  port?: number;
}
